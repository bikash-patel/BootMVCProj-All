...
public class EmployeeController {
	...
	@PostMapping("/search")
	public String searchWithDynamicSearch(@PageableDefault(page = 0,size = 5,sort="empid",direction =Direction.ASC) Pageable pageable,
										  Map<String,Object> map,
										  @ModelAttribute("emp") Employee employee) 
	{
		Page<Employee> list=service.searchEmployeeWithDynamicSearch(employee,pageable);
		map.put("employee",list.getContent());
		map.put("page",list);
		return "report";
	}
	
	@GetMapping("/report")
	public String reportPage(@PageableDefault(page = 0,size = 5,sort="empid",direction =Direction.ASC) Pageable pageable,
							Map<String,Object> map,
							@ModelAttribute("emp") Employee employee) 
	{
		Page<Employee> page=service.getAllEmployeeWithPagination(pageable);
		map.put("employee",page.getContent()); // Here we can not pass directly page because it will be not
					 //not iterate by forEach, for that we need to use getContent method
		map.put("page",page);
	...
	@GetMapping("/edit")
	public String editPage(@RequestParam("id") int id,@ModelAttribute Employee employee) 
	...
	@GetMapping("/delete")
	public String deleteOperation(@RequestParam("id") int id,Map<String,Object> map) 
	{
		String msg=service.deleteEmployee(id);
		map.put("result",msg);
		return "forward:report"; //Here source and destination both request having save mode GET so
